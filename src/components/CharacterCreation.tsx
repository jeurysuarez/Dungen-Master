...
const CharacterCreation: React.FC<CharacterCreationProps> = ({ onCharacterCreate }) => {
  ...
  useEffect(() => {
    ...
    if (defaults.skills) {
        defaults.skills.forEach(skill => {
            initialCooldowns[skill.name] = skill.cooldown;
        });
    }
    setSkillCooldowns(initialCooldowns);
  }, [charClass]);

  const handleCooldownChange = (skillName: string, change: number) => {
    setSkillCooldowns(prev => {
        const currentCooldown = prev[skillName] || 0;
        const newCooldown = Math.max(MIN_COOLDOWN, Math.min(MAX_COOLDOWN, currentCooldown + change));
        return { ...prev, [skillName]: newCooldown };
    });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('');
    const defaults = CLASS_DEFAULTS[charClass];
    const customizedSkills = defaults.skills.map(skill => ({
        ...skill,
        cooldown: skillCooldowns[skill.name] || skill.cooldown,
    }));
  ...
  return (
    <div className="relative z-20 w-full max-w-md mx-auto flex items-center justify-center h-full py-8">
        <div className="bg-gray-800/50 border border-amber-600/30 rounded-lg p-8 shadow-lg backdrop-blur-sm w-full max-h-full overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {selectedClassStats.skills.length > 0 && (
                  <div>
                    <label className="block mb-2 text-sm font-bold text-gray-400">Personalizar Habilidades</label>
                    <div className="space-y-3 p-3 bg-black/20 rounded-md">
                      {selectedClassStats.skills.map(skill => (
                        <div key={skill.name}>
                          <p className="font-bold text-gray-200 text-sm">{skill.name}</p>
                          <div className="flex items-center justify-between">
                            <p className="text-xs text-gray-400 italic pr-2 flex-1">"{skill.description}"</p>
                            <div className="flex items-center gap-2">
                              <label className="text-xs text-gray-400">Enfriamiento:</label>
                              <button type="button" onClick={() => handleCooldownChange(skill.name, -1)} className="w-6 h-6 bg-gray-700 rounded text-lg flex items-center justify-center hover:bg-gray-600">-</button>
                              <span className="w-8 text-center font-bold text-amber-300">{skillCooldowns[skill.name]}</span>
                              <button type="button" onClick={() => handleCooldownChange(skill.name, 1)} className="w-6 h-6 bg-gray-700 rounded text-lg flex items-center justify-center hover:bg-gray-600">+</button>
                            </div>
                          </div>
                        </div>
